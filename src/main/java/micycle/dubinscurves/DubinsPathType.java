...
/**
 * Represents the individual types of segments that can form a Dubins path. A
 * Dubins path consists of three such segments. This enum is package-private as
 * it's an internal detail used by {@link DubinsPathType}.
 */
enum SegmentType {
 ...
 * <p>
 * A Dubins path is the shortest curve connecting two configurations (position
 * and orientation) in a 2D plane, subject to a constraint on the path's
 * curvature. These paths are constructed by concatenating three segments, where
 * each segment is either a circular arc (Left (L) or Right (R) turn at a
 * constant radius) or a straight line (S).
 * <p>
 ...
 * </ul>
 * This enumeration provides these types and the sequence of {@link SegmentType}
 * they correspond to.
 */
public enum DubinsPathType {
	 ...
	 */
	private DubinsPathType(final SegmentType[] types) {
		this.segmentTypes = types;
	}

	/**
	 * Returns the sequence of {@link SegmentType} that defines this Dubins path
	 * type. This is primarily for internal use by the Dubins path generation logic.
	 *
