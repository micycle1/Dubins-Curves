 ...
 * There are six fundamental types of Dubins paths: LSL, LSR, RSL, RSR, RLR,
 * LRL.
 * <p>
...
public class DubinsPath {
	...
	/**
	 * The initial configuration (x, y, theta) of the path. Populated by the
	 * constructor.
	 */
	public double[] configStart = new double[3];
	/**
	 * The lengths of the three segments of the path, before scaling by rho. These
	 * are the "normalized" segment lengths. Populated by the underlying
	 * DubinsCurves library.
	 */
	double[] segmentLengths = new double[3]; // package-private for direct access by DubinsCurves
	/**
	 * The turning radius (rho) of the path. This is equivalent to (forward velocity
	 * / maximum angular velocity). Populated by the constructor.
	 */
	public double rho;
	/**
	...
	/**
	 * Private constructor to initialize internal structures. Not intended for
	 * direct public use.
	 */
	private DubinsPath() {
	}
	 ...
	 * configuration.
	 * <p>
	 * A configuration is an array <code>[x, y, theta]</code>, where
	 * <code>(x, y)</code> is the Cartesian coordinate and <code>theta</code> is the
	 ...
	 *                    value. This is typically calculated as (forward velocity /
	 ...
	 *                    value. This is typically calculated as (forward velocity /
	 *                    maximum angular velocity).
	 * @param pathType    The specific {@link DubinsPathType} to generate (e.g.,
	 *                    LSL, RSR).
	...
	/**
	 * Returns the total length of this Dubins path. The length is the sum of the
	 * lengths of its three segments.
	 *
	...
	/**
	 * Returns the length of a specific segment of the path. Each Dubins path
	 * consists of three segments.
	 *
	 * @param segment The index of the segment (0, 1, or 2). 0: first segment, 1:
	 *                second segment, 2: third segment.
	 * @return The length of the specified segment. Returns 0 if the segment index
	...
	/**
	 * Returns the "normalized" length of a specific segment from the path. The
	 ...
	 *
	 * @return The {@link DubinsPathType} enum representing the path's type (e.g.,
	 *         LSL, RSR). May return null if the path could not be successfully
	 *         computed (e.g., due to invalid turning radius).
	 */
	...
	/**
	 * Samples configurations at regular intervals along the path. This method
	 * iterates along the path, invoking the provided callback function at each
	 * <code>stepSize</code> increment.
	 * <p>
	 * If the endpoint falls on a step, it is included.
	 * <p>
	 * The sampling process continues until the entire path has been sampled or the
	 * callback function returns a non-zero value, which signals to stop sampling.
	 *
	 * @param stepSize The distance along the path between successive samples. Must
	 *                 be positive.
	 * @param callback A {@link DubinsPathSamplingCallback} function that will be
	 *                 invoked for each sampled configuration. The callback receives
	 *                 the configuration <code>[x, y, theta]</code> and the
	 ...
	 * The extracted path starts at this path's <code>configStart</code> and has a
	 * total length of <code>t</code>.
	 *
	 * @param t The desired length of the sub-path to extract. Must be non-negative
	 *          and not greater than the total length of this path. If
	 *          <code>t</code> is 0, an empty path is returned. If <code>t</code>
	 *          exceeds the path length, the returned path will be equivalent to the
	 *          original path.
